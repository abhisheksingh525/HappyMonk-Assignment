...
with open("thermal_annotations.json") as f:
    train_data = json.loads(f.read())
    for annotation in train_data["annotations"]:
        train_annotations.append(annotation)

with open("thermal_annotations_val.json") as f:
...
def create_dataset(thermal_dataset, dataset_type):
    ...
    initial_count=count_number_of_images("Users\AVANISH SINGH\train\thermal_8_bit")
    print(initial_count)
    ppath=f"Users\AVANISH SINGH"
    for coun, row in enumerate(tqdm(thermal_dataset)):
        ...
        if dataset_type == "train":
            ...
            print(image_name)
            imgPath = ppath+"\\{}\\thermal_8_bit\\FLIR_{}.jpeg".format(dataset_type,str(row["image_id"]+1).zfill(5))
